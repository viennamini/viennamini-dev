...
//    QuantityType & srh_p1                   = problem_description.add_quantity(viennamini::id::srh_p1());
        ...
        sit != segmesh.segmentation.end(); ++sit)
    {
      ...
      if(device().is_semiconductor(current_segment_index))
      {
        ...
        if(device().get_mobility(current_segment_index) == mobility::none)
        {
        #ifdef VIENNAMINI_VERBOSE
...
//    EquationType cont_eq_p  = viennamath::make_equation( viennamath::div(mu_p * VT * viennamath::grad(p) + mu_p * viennamath::grad(psi) * p) - R_srh_left, /* = */ R_srh_right);
    ...
    electron_density_cm.scale_values(1.0E-6);

    QuantityType & hole_density_cm        = problem_description.add_quantity(hole_density);
    hole_density_cm.set_name(viennamini::id::hole_density()+"_cm");
    hole_density_cm.scale_values(1.0E-6);
    ...
    if(step_id == 1) // the first simulation has the index 1
    {
          ...
          sit != segmesh.segmentation.end(); ++sit)
      {
        ...
        if(device().is_contact(current_segment_index))
        {
          data_table().new_column( "U_"+device().get_name(current_segment_index) );

          if(device().is_contact_at_semiconductor(current_segment_index))
          {
            data_table().new_column( "I_"+device().get_name(current_segment_index) );
          }
        }
      }
    }
        ...
        sit != segmesh.segmentation.end(); ++sit)
    {
      ...
      if(device().is_contact(current_segment_index))
      {
        data_table().add("U_"+device().get_name(current_segment_index), 
                          current_contact_potentials[current_segment_index]);
        ...
        if(device().is_contact_at_semiconductor(current_segment_index))
        {
          ...
          data_table().add("I_"+device().get_name(current_segment_index), 
